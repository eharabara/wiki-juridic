"""Verificare dupa ingerarea legilor bancare in romana (P8). GENERAT din
_meta/imports/moldova-legal/verify_business_law.py la 2026-09-05, cu trei inlocuiri: DOCS din
ingest_bnm_ro.py, RAW = raw/papers/bnm/legal-ro, META = _meta/imports/bnm/legis-md-ro. Testul de
fond este acelasi: se scot liniile de structura adaugate si restul se compara linie cu linie cu
extractia simpla din acelasi HTML.
"""
...
spec = importlib.util.spec_from_file_location("ibl", str(Path(__file__).parents[1] / "moldova-legal" / "ingest_business_law.py"))
_w = importlib.util.spec_from_file_location("w", str(Path(__file__).parent / "ingest_bnm_ro.py"))
