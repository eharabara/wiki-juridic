...
Generated from `_meta/schema/schema-spec.yaml` (version 2026-09-06). Every rule below is checked by `python _meta/schema/validate_wiki.py`. To change a rule, edit the spec, rerun `python _meta/schema/build_schema.py`, then run the validator.
...
- **document and knowledge types:** `entity`, `concept`, `comparison`, `query`, `summary`, `timeline`, `source-note`, `needs-links`, `contested`, `unverified`
