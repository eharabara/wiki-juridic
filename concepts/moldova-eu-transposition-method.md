...
# Metoda de transpunere a acquis-ului UE în dreptul Republicii Moldova
...
Înainte de redactare, stabilește natura intervenției:
...
Pentru fiecare transpunere, adună și notează explicit:
...
| Etapă | Scop | Output minim |
|---|---|---|
| 1. Identificare act UE | stabilește exact actul/actele ce trebuie transpuse | listă CELEX + versiuni consolidate |
| 2. Citire normativă UE | extrage obiective, principii, definiții, obligații, opțiuni și termene | fișă de obligații UE |
| 3. Mapare drept MD | identifică norme existente și grad preliminar de compatibilitate | tabel preliminar UE → MD |
| 4. Arhitectură normativă | decide lege nouă, lege de modificare, HG, regulament autoritate, norme tranzitorii | schelet de arhitectură normativă |
| 5. Matrice de concordanță | demonstrează articol-cu-articol transpunerea sau lacuna | tabel de concordanță completabil |
| 6. Control procedural | aplică [[moldova-eu-transposition-rule-matrix]] | checklist cu reguli high/medium |
| 7. Output decizional | livrează gap analysis, recomandări, întrebări deschise și riscuri | notă juridico-policy / draft skeleton |

## Arhitectura normativă

Alegerea instrumentului nu se face doar după domeniu, ci după trei întrebări:

1. **Forța juridică necesară:** obligația UE cere norme primare sau poate fi implementată prin act subordonat?
...
Pentru orice solicitare viitoare de transpunere, răspunsul/documentul de lucru trebuie să conțină cel puțin:

1. **Source pack** — ce surse au fost folosite și ce versiune;
2. **EU obligation map** — obligații, opțiuni, definiții, termene;
3. **MD mapping** — norme existente, lipsuri, instituții competente;
4. **Concordance skeleton** — articol UE → normă MD → grad → acțiune;
5. **Normative architecture** — instrument juridic recomandat și modificări conexe;
6. **Procedural checklist** — reguli aplicate din [[moldova-eu-transposition-rule-matrix]];
7. **Risks/open questions** — competență, gold-plating, sub-transpunere, termeni UE, perioade tranzitorii, implementare instituțională.
...
- [[acquis-CSDR-EMIR]], [[acquis-MiFID]], [[acquis-MAR]] — exemple de acquis unde metoda va fi folosită.

## Întrebări deschise

- De ingerat separat metodologia/manualul Cancelariei de Stat privind armonizarea, pentru detalii de completare a tabelului de concordanță și expertizei de compatibilitate.
- De creat ulterior un skill procedural, numai după ce această metodă și matricea de reguli sunt stabile în wiki.
